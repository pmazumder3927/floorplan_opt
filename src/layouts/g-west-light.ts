 ...
 * (50.8% falls to 46.5%, which is below layout A), so it is priced and drawn as
 * an OPTION rather than folded into a scheme whose brief was "too dark".
 *
...
const layout: Layout = {
  ...
  notes: [
    ...
    'THE WALL, AND WHY IT IS AN OPTION RATHER THAN A DECISION. 85-88 sq ft of the bathroom partition\'s west face in Sherwin-Williams Urbane Bronze SW 7048 — 8% LRV, warm brown-charcoal, flat, floor to soffit, corner to corner, about $205 if the client paints it and $300-$450 if a painter does. IT WAS DRAWN INTO THIS SCHEME FIRST AND THEN TAKEN OUT, and the number is why: with the paint the room measures 46.5% area-weighted, without it 50.8%, and layout A is 48.0%. A client who said "too dark" would be handed a room measurably darker than the one they complained about. WHAT IT BUYS IS REAL AND IT IS NOT IN THAT NUMBER. A 100" image is judged against its surround, and 55 sq ft of 88%-LRV white a half-inch from the picture\'s edge raises the eye\'s adaptation level and crushes perceived black in a way no contrast ratio records; a 0.22:1 lens also grazes that wall at a few degrees, so on white every drywall butt joint is lit like a raking-light survey. THE OBJECTION THAT SETTLES IT EITHER WAY: the ALR fabric is 26.6% LRV, so on an 8% wall the switched-OFF screen becomes three times brighter than the wall behind it — the picture stops being a dark rectangle on a light wall and becomes a light rectangle on a dark one. It never disappears; the paint only chooses which way it stands out. So: a 2 ft sample board taped to that wall, looked at once in daylight and once at 8pm with the downlights on, and then a decision. It is one wall of flat latex and it repaints in an afternoon — and it is still somebody else\'s wall, so ask.',
