 ...
 * EAST of them. A 27.6"-deep top fits it with 3 1/4" to spare, so the alcove
...
const DESK_W = 59.1 / 12; // 4.925 — 59.1" of top, east-west
const DESK_D = 27.6 / 12; // 2.3 — 27.6"; a 30" top would leave under 1" at the front
const DESK_X = 4.5; // top runs x 2'-0 7/16" .. 6'-11 9/16"
const DESK_Y = N_FACE + 0.02 + DESK_D / 2; // 1.80 -> y 0.65 .. 2.95
...
const CHAIR_Y = DESK_FRONT + 0.03 + CHAIR_D / 2; // 3.9696 -> y 2.98 .. 4.96
...
const layout: Layout = {
  ...
  name: 'E — Clear shot',
  description:
    'The sightline scheme: the desk moves into the north-west notch — the only floor in this plate no seat-to-screen ray crosses — and the queen goes onto the wall, so sixteen hours a day there is no bed in the apartment at all. 100% of the picture from every seat, a sofa dead on the screen centreline, no errors and no warnings, and $3,636 off layout A.',
  plan: 'studio-508',
  items: [
    // ========================================================== PROJECTION
    {
    ...
    // ================================================================ WORK
    {
      ...
      note: 'IN THE NOTCH, which is the whole scheme. Back to the alcove\'s north wall, user facing north, panel facing south, west glazing on the user\'s left — the faces.ts rule, obeyed, and here it also puts the user\'s back to the room. 27.6" deep rather than 30" because the alcove is 2\'-7 1/8" deep: a 30" top would leave under 1" at the front and this one leaves 3 1/4". IT REPLACED AN OAK-LAMINATE JARVIS AND THE TRADE IS SMALL HERE: 0.9" narrower, 0.6" deeper, $526 cheaper, and dark instead of pale. In a PALE alcove a dark top is a bigger visual change than it is anywhere else in this project — but this layout is also the one whose whole point is that the study sits outside every seat-to-screen ray, so a dark top is doing less contrast work here than in A or C and is mostly a colour decision.',
    },
    {
      id: 'desk-arm',
    ...
    // No separate tray line: the MAGNUS has the tray, the routing and a mains
    // socket built into its own frame.
    ...
    },
    {
      ...
      note: 'OVER THE CHEST, NOT OVER THE DESK, and that is a sit-stand consequence rather than a preference: the MAGNUS top rises to 49 1/4" and the panel on its arm rides with it to about 71", so ANY shelf over that desk is either unreachable or in the way of the top. Over the 35 3/8" chest nothing moves, so 48" is simply the right height — the shelf occupies 48" to 67 3/4" with 12 5/8" of clear air above the chest. $230 against the $645 String 3-bay the earlier layouts carried.',
    },
    {
      id: 'chest',
